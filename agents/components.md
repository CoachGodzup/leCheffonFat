...
AnimatedIcons,src/components/atoms/AnimatedIcons/AnimatedIcons.tsx,None,"Fixed fullscreen layer of food icons (Lucide) that wander between 64×64 grid cells. aria-hidden."
...
Toast,src/components/atoms/Toast/Toast.tsx,"{ toast: Toast; onClose: (id: string) => void }","Single toast notification with auto-dismiss, close button, role=""alert"". Supports error and warning types via data-type."
```
...
ToastContainer,src/components/molecules/ToastContainer/ToastContainer.tsx,None,"Fixed top-right container that renders active toasts from notification-store."
```
